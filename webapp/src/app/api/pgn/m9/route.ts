...
export async function GET() {
  try {
    ...
    // Query for eval_mate_in between 7 and 9
    const response = await databases.listDocuments(
      DATABASE_ID,
      COLLECTION_ID,
      [
        Query.greaterThanEqual("eval_mate_in", 7),
        Query.lessThanEqual("eval_mate_in", 9)
      ]
    );

    if (response.documents.length === 0) {
      return NextResponse.json(
        { error: 'No FEN data found for eval_mate_in range 7-9' },
        { status: 404 }
      );
    }
    ...
    console.error('Error fetching FEN data (7-9):', error);
    return NextResponse.json(
