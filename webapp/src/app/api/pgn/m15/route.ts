...
export async function GET() {
  try {
    ...
    // Query for eval_mate_in between 13 and 15
    const response = await databases.listDocuments(
      DATABASE_ID,
      COLLECTION_ID,
      [
        Query.greaterThanEqual("eval_mate_in", 13),
        Query.lessThanEqual("eval_mate_in", 15)
      ]
    );

    if (response.documents.length === 0) {
      return NextResponse.json(
        { error: 'No FEN data found for eval_mate_in range 13-15' },
        { status: 404 }
      );
    }
    ...
    console.error('Error fetching FEN data (13-15):', error);
    return NextResponse.json(
