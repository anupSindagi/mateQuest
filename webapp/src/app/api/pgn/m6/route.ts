...
export async function GET() {
  try {
    ...
    // Query for eval_mate_in between 4 and 6
    const response = await databases.listDocuments(
      DATABASE_ID,
      COLLECTION_ID,
      [
        Query.greaterThanEqual("eval_mate_in", 4),
        Query.lessThanEqual("eval_mate_in", 6)
      ]
    );

    if (response.documents.length === 0) {
      return NextResponse.json(
        { error: 'No FEN data found for eval_mate_in range 4-6' },
        { status: 404 }
      );
    }
    ...
    console.error('Error fetching FEN data (4-6):', error);
    return NextResponse.json(
