...
export async function GET() {
  try {
    ...
    // Query for eval_mate_in between 10 and 12
    const response = await databases.listDocuments(
      DATABASE_ID,
      COLLECTION_ID,
      [
        Query.greaterThanEqual("eval_mate_in", 10),
        Query.lessThanEqual("eval_mate_in", 12)
      ]
    );

    if (response.documents.length === 0) {
      return NextResponse.json(
        { error: 'No FEN data found for eval_mate_in range 10-12' },
        { status: 404 }
      );
    }
    ...
    console.error('Error fetching FEN data (10-12):', error);
    return NextResponse.json(
